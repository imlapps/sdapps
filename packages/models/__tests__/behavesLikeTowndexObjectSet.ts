...
export function behavesLikeTowndexObjectSet(objectSet: $ObjectSet): void {
  ...
  it("event count", async ({ expect }) => {
    expect((await objectSet.eventsCount()).unsafeCoerce()).toEqual(113);
  });

  it("organization stubs", async ({ expect }) => {
    expect(Either.rights(await objectSet.organizationStubs())).toHaveLength(3);
  });

  it("organization count", async ({ expect }) => {
    expect((await objectSet.organizationsCount()).unsafeCoerce()).toEqual(3);
  });

  it("person stubs", async ({ expect }) => {
    expect(Either.rights(await objectSet.personStubs())).toHaveLength(52);
  });

  it("person count", async ({ expect }) => {
    expect((await objectSet.peopleCount()).unsafeCoerce()).toEqual(52);
  });
}
